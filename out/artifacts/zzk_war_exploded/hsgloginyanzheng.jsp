...
<html>
 ...
 if(cx.equals("志愿团队")){sql="select * from zhiyuantuandui where tuanduizhanghao='"+uid+"' and mima='"+pwd+"' and issh='是'";}if(cx.equals("志愿者")){sql="select * from zhiyuanzhe where zhiyuanzhezhanghao='"+uid+"' and mima='"+pwd+"' and issh='是'";}if(cx.equals("注册用户")){sql="select * from yonghuzhuce where yonghuming='"+uid+"' and mima='"+pwd+"' and issh='是'";}
	...
	out.print("<script>alert('你输入的用户不存在或密码错误,请重新登录!');window.history.go(-1);</script>");
	 ...
	 if(cx.equals("管理员"))
	 {
	  ...
	  else
	  {
	  	...
	  	session.setAttribute("aaa",RS_result.getString("youxiang"));  
	  }
	 if(cx.equals("管理员")){
