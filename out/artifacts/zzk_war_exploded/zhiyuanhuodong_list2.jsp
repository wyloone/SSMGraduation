...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
			  ...
			  <table width="100%" border="1" align="center" cellpadding="4" cellspacing="1" bgcolor="#aec3de">
					...
					<tr align="right" bgcolor="#ebf0f7">
					  ...
					  <table width="100%" border="0" align="center" cellpadding="4" cellspacing="1" >
						<tr align="left" >
							<td colspan="17">
								<form action="zhiyuanhuodongList2.do" name="myform" method="post">
                  ...
                  <tr align="center">
						<td style="padding-left:0px; padding-right:0px;"><table width="100%" border="1" align="center" cellpadding="4" cellspacing="1" bgcolor="#ebf0f7" class="table table-striped table-bordered table-hover">
                          <tr>
							   ...
							   <td align='center' bgcolor='#ebf0f7' width="10%">活动报名结束时间</td>
							  ...
							  <%-- <td align='center' bgcolor='#ebf0f7' width="5%">团队账号</td>
							   ...
							   <td align='center' bgcolor='#ebf0f7' width="6%">活动状态</td>
							   <td  align='center' width="7%" bgcolor='#ebf0f7'>审核回复</td>
                          ...
                          <c:forEach items="${list }" var="u">
    						...
    						<td align='center' bgcolor='#ebf0f7'>${u.issh}</td><td>${u.shhf}</td>
								<%--<a class="btn btn-info btn-small" href="sh2.jsp?id=${u.id}&tablename=zhiyuanhuodong">${u.issh}</a></td><td>${u.shhf}</td>--%>
							<td height="45" align="center"> ${u.addtime } </td>
                              <td height="45" align="center">
								 ...
								 <%-- <a class="btn btn-info btn-small" href="deleteZhiyuanhuodong.do?id=${u.id }" onclick="{if(confirm('确定要删除吗?')){return true;}return false;}">删除</a>--%>
                                     <%--根据活动状态判断后面的操作--%>
                                      <c:if test="${u.issh.equals('完成')}">
										  <a class="btn btn-info btn-small" onclick="javascript:location.href='baoming_listxls.jsp?name=${u.huodongmingcheng}'">志愿者名单</a>
                                         <a class="btn btn-info btn-small" href="zyhdDetail.do?id=${u.id }" target="_blank">查看详情</a>
                                     </c:if>
                                      <c:if test="${u.issh.equals('待审核') || u.issh.equals('进行中') || u.issh.equals('未通过')}">
                                          <a class="btn btn-info btn-small" href="zyhdDetail.do?id=${u.id }" target="_blank">查看详情</a>
                                      </c:if>
							  </td>
                            </tr>
		 ...
		 <c:if test="${page.page>1}">
              <a href="zhiyuanhuodongList2.do?page=1" >首页</a>
             <a href="zhiyuanhuodongList2.do?page=${page.page-1 }"> 上一页</a>             </c:if>
    	     <c:if test="${page.page<page.totalPage}">
			<a href="zhiyuanhuodongList2.do?page=${page.page+1 }">下一页</a>
			<a href="zhiyuanhuodongList2.do?page=${page.totalPage }">末页</a>		    </c:if>		
	</c:if>
	</p>
	...
	<%
		...
		String sql="";
		sql="select count(id) as gwegwg  from zhiyuanhuodong where issh='待审核'and tuanduizhanghao='"+request.getSession().getAttribute("username")+"'";
	...
	<div id="winpop">
		...
		<div class="con">当前<%=aa%>条活动待审核！</div>
	</div>
	</body>
</html>
